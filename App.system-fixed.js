...
const BraceletCard = React.memo(({ bracelet, onPress, safetyRadius }) => {
  ...
  const hasCriticalAlerts = bracelet.alerts && bracelet.alerts.some(alert => alert.level === 'critical');
  const hasWarnings = bracelet.alerts && bracelet.alerts.some(alert => alert.level === 'warning');
        ...
        >
          ...
          <View style={styles.braceletMetrics}>
            <View style={styles.metricItem}>
              ...
              <Text style={styles.metricValue}>{bracelet.distance.toFixed(2)}m</Text>
            </View>
            ...
            <View style={styles.metricItem}>
              ...
              <Text style={styles.metricValue}>{bracelet.batteryLevel.toFixed(2)}%</Text>
            </View>
          ...
          {/* Alertes */}
          {bracelet.alerts && bracelet.alerts.length > 0 && (
...
const Dashboard = ({ bracelets, alerts, safetyRadius, onBraceletPress }) => {
    ...
    if (selectedFilter !== 'all') {
      switch (selectedFilter) {
        ...
        case 'low_battery':
          ...
          break;
        case 'critical':
          filtered = filtered.filter(b => b.alerts && b.alerts.some(a => a.level === 'critical'));
          break;
        case 'out_of_range':
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      lowBattery: braceletsList.filter(b => b.batteryLevel <= SYSTEM_CONFIG.ALERTS.LOW_BATTERY).length,
      criticalBattery: braceletsList.filter(b => b.batteryLevel <= SYSTEM_CONFIG.ALERTS.CRITICAL_BATTERY).length,
      outOfRange: braceletsList.filter(b => b.distance > safetyRadius).length,
      averageSignal: braceletsList.length > 0 ? braceletsList.reduce((acc, b) => acc + b.signalBars, 0) / braceletsList.length : 0
    };
  }, [bracelets, safetyRadius]);
      ...
      <View style={styles.braceletsSection}>
        ...
        {filteredAndSortedBracelets.map(bracelet => (
          <BraceletCard
            key={bracelet.id}
            bracelet={bracelet}
            onPress={() => onBraceletPress && onBraceletPress(bracelet)}
  ...
  const updateBraceletsData = useCallback(() => {
    setBracelets(prevBracelets => 
      prevBracelets.map(bracelet => {
        ...
        // Simulation de décharge batterie avec 2 décimales
        const batteryDrain = parseFloat((Math.random() * 0.3).toFixed(2));
        const newBattery = parseFloat(Math.max(0, bracelet.batteryLevel - batteryDrain).toFixed(2));
        ...
        // Simulation de variations de signes vitaux avec 2 décimales
        ...
        return {
          ...
          temperature: parseFloat(Math.max(35, Math.min(40, bracelet.temperature + temperatureVariation)).toFixed(2)),
      ...
      <View style={styles.container}>
        ...
        <ScrollView style={styles.detailContainer}>
          ...
          <View style={styles.detailCard}>
            ...
            <View style={styles.metricsGrid}>
              ...
              <View style={styles.metricDetail}>
                ...
                <Text style={styles.metricDetailValue}>{selectedBracelet.batteryLevel.toFixed(2)}%</Text>
              </View>
              ...
              <View style={styles.metricDetail}>
                ...
                <Text style={styles.metricDetailValue}>{selectedBracelet.temperature.toFixed(2)}°C</Text>
              </View>
            </View>
          </View>
          ...
          <View style={styles.detailCard}>
            ...
            {selectedBracelet.alerts && selectedBracelet.alerts.length > 0 ? (
