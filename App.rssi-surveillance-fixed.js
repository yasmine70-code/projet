...
const SYSTEM_CONFIG = {
  ...
  RSSI_THRESHOLDS: {
    ...
    good: -50,        // 4 barres - 10-20m
    fair: -65,        // 3 barres - 20-35m
    weak: -80,        // 2 barres - 35-50m
    very_weak: -95,   // 1 barre - 50-100m
    none: -110        // 0 barre - >100m
  },
  DISTANCE_ESTIMATES: {
    excellent: { min: 0, max: 10, color: '#22c55e', label: 'Très Proche', bars: 5 },
    good: { min: 10, max: 20, color: '#3b82f6', label: 'Proche', bars: 4 },
    fair: { min: 20, max: 35, color: '#f59e0b', label: 'Moyen', bars: 3 },
    weak: { min: 35, max: 50, color: '#fb923c', label: 'Loin', bars: 2 },
    very_weak: { min: 50, max: 100, color: '#ef4444', label: 'Très Loin', bars: 1 },
    none: { min: 100, max: 150, color: '#6b7280', label: 'Hors Portée', bars: 0 }
  },
...
const getSignalBarsFromRSSI = (rssi) => {
...
const getSignalBarsColor = (bars) => {
  if (bars >= 4) return '#22c55e';
  if (bars >= 3) return '#3b82f6';
  if (bars >= 2) return '#f59e0b';
  if (bars >= 1) return '#fb923c';
  return '#6b7280';
};

// Composant de barres de signal WiFi selon le cahier des charges
function SignalBarsIndicator({ bars, rssi, distance }) {
  const barColors = [
    '#6b7280', // 0 barres - gris
    '#ef4444', // 1 barre - rouge
    '#fb923c', // 2 barres - orange
    '#f59e0b', // 3 barres - jaune
    '#3b82f6', // 4 barres - bleu
    '#22c55e'  // 5 barres - vert
  ];
  ...
  return (
    <View style={styles.signalBarsContainer}>
        ...
        {[1, 2, 3, 4, 5].map((bar) => (
          <Animated.View
            ...
            style={[
              styles.signalBar,
              {
                height: bar * 4, // Hauteur progressive: 4, 8, 12, 16, 20
                backgroundColor: bar <= bars ? barColors[bars] : '#e5e7eb',
                opacity: bar <= bars ? 1 : 0.3,
                transform: [
                  { scale: bar <= bars && bars <= 2 ? pulseAnim : 1 }
                ]
              }
            ]}
          />
      ...
      <View style={styles.signalInfo}>
        <Text style={[styles.signalBarsText, { color: getSignalBarsColor(bars) }]}>
          {bars}/5
        </Text>
        <Text style={styles.signalDistanceText}>
...
function ProximityIndicator({ proximity, distance, rssi }) {
  ...
  const bars = getSignalBarsFromRSSI(rssi);
  ...
  return (
    <View style={styles.proximityContainer}>
      ...
      <SignalBars bars={bars} rssi={rssi} distance={distance} />
    </View>
  );
}
...
// Composant simplifié des barres de signal
function SignalBars({ bars, rssi, distance }) {
  const barColors = [
    '#6b7280', // 0 barres
    '#ef4444', // 1 barre
    '#fb923c', // 2 barres
  ...
  return (
    <View style={styles.miniSignalBars}>
      {[1, 2, 3, 4, 5].map((bar) => (
        <View
          key={bar}
          style={[
...
function LoginScreen({ onLoginSuccess }) {
  ...
  useEffect(() => {
    // Simulation d'initialisation du réseau WiFi AP avec gestion d'erreurs
    const initializeSystem = async () => {
      try {
        setSystemErrors([]);
        
        // Étape 1: Initialisation matériel
        await new Promise(resolve => setTimeout(resolve, 800));
        
        // Étape 2: Configuration WiFi AP
        await new Promise(resolve => setTimeout(resolve, 600));
        
        // Étape 3: Démarrage du point d'accès
        await new Promise(resolve => setTimeout(resolve, 600));
        ...
        setSystemErrors(['Erreur initialisation système']);
        setWifiStatus('error');
      }
    };
  ...
  const handleStartSystem = () => {
    ...
    if (!apConfig) {
      errors.push('Configuration WiFi non initialisée');
    }
    
    if (ESP32_BRACELETS.length === 0) {
      errors.push('Aucun bracelet détecté');
    }
    ...
    setTimeout(() => {
      if (Math.random() > 0.1) { // 90% de succès
        onLoginSuccess({ 
          ...
          ap_config: apConfig,
          active_bracelets: ESP32_BRACELETS.length
        });
      } else {
        setSystemErrors(['Échec démarrage système']);
        setIsLoading(false);
      }
    }, 2000);
  };
        ...
        <Animated.View style={[styles.loginContainer, { opacity: fadeAnim }]}>
          ...
          <View style={styles.header}>
            ...
            <View style={styles.statusContainer}>
              <View style={[
                ...
                { 
                  backgroundColor: wifiStatus === 'ready' ? '#22c55e' : 
                                 wifiStatus === 'error' ? '#ef4444' : '#f59e0b'
                }
              ]} />
              <Text style={styles.statusText}>
                {wifiStatus === 'initializing' ? 'Initialisation WiFi AP...' :
                 wifiStatus === 'ready' ? 'WiFi AP Prêt' :
                 wifiStatus === 'error' ? 'Erreur Système' : 'État Inconnu'}
              </Text>
            </View>
            ...
            {apConfig && (
              <View style={styles.configContainer}>
                ...
                <Text style={styles.configItem}>Mise à jour: {SYSTEM_CONFIG.UPDATE_INTERVAL/1000}s</Text>
                <Text style={styles.configItem}>Bracelets actifs: {ESP32_BRACELETS.length}</Text>
              </View>
            )}
          </View>
          ...
          <View style={styles.form}>
            ...
            <View style={styles.infoContainer}>
              ...
              <Text style={styles.infoItem}>✅ Affichage barres de signal (1-5)</Text>
...
function HomeScreen({ user, onLogout }) {
  ...
  // Simulation de mise à jour RSSI toutes les 5 secondes avec gestion d'erreurs
  useEffect(() => {
      ...
      try {
        setBracelets(prevBracelets => 
          prevBracelets.map(bracelet => {
            ...
            const newRssi = Math.max(-120, Math.min(-20, bracelet.rssi + rssiVariation));
            ...
            const signalBars = getSignalBarsFromRSSI(newRssi);
            
            // Simulation de perte de connexion
            const connectionStatus = newRssi < -110 ? 'disconnected' : 'connected';
        ...
        setSystemErrors([]); // Effacer les erreurs si mise à jour réussie
      } catch (error) {
        setSystemErrors(['Erreur mise à jour des données']);
      }
    ...
    setTimeout(() => {
      try {
        ...
        setBracelets(prevBracelets => 
          prevBracelets.map(bracelet => ({
            ...bracelet,
            rssi: Math.max(-120, Math.min(-20, bracelet.rssi + (Math.random() - 0.5) * 15)),
            signal_bars: getSignalBarsFromRSSI(bracelet.rssi + (Math.random() - 0.5) * 15),
            last_seen: new Date()
          }))
        );
        setRefreshing(false);
      } catch (error) {
        setSystemErrors(['Erreur lors du rafraîchissement']);
        setRefreshing(false);
      }
    }, 2000);
  };
  ...
  const getFilteredBracelets = () => {
    try {
      switch (selectedFilter) {
        ...
        case 'out_of_range':
          return bracelets.filter(b => b.proximity === 'none' || b.proximity === 'very_weak');
        case 'strong_signal':
          return bracelets.filter(b => b.signal_bars >= 4);
        case 'weak_signal':
          return bracelets.filter(b => b.signal_bars <= 2);
        default:
          return bracelets;
      }
    } catch (error) {
      setSystemErrors(['Erreur filtrage des bracelets']);
      return bracelets;
    }
  };
  ...
  const checkAlerts = () => {
    ...
    try {
      ...
      // Alertes signal faible
      const weakSignalBracelets = bracelets.filter(b => b.signal_bars <= 1);
      if (weakSignalBracelets.length > 0) {
        alerts.push({
          type: 'signal',
          message: `${weakSignalBracelets.length} bracelet(s) avec signal très faible`,
          severity: 'medium'
        });
      }
    } catch (error) {
      alerts.push({
        type: 'system',
        message: 'Erreur vérification des alertes',
        severity: 'high'
      });
    }
  ...
  return (
    <View style={styles.container}>
      ...
      >
          ...
          >
            <View style={styles.userInfo}>
              ...
              <View style={styles.userDetails}>
                ...
                <Text style={styles.activeBracelets}>
                  Bracelets actifs: {user?.active_bracelets || ESP32_BRACELETS.length}
                </Text>
              </View>
            </View>
        ...
        {/* Affichage des erreurs système */}
        {systemErrors.length > 0 && (
          <View style={styles.systemErrorsSection}>
            <Text style={styles.sectionTitle}>⚠️ Erreurs Système</Text>
            {systemErrors.map((error, index) => (
              <View key={index} style={styles.systemErrorItem}>
                <Ionicons name="warning" size={20} color="#ef4444" />
                <Text style={styles.systemErrorText}>{error}</Text>
              </View>
            ))}
          </View>
        )}
        ...
        <View style={styles.filtersSection}>
          ...
          >
            {['all', 'connected', 'disconnected', 'sos', 'low_battery', 'out_of_range', 'strong_signal', 'weak_signal'].map((filter) => (
              <TouchableOpacity
                key={filter}
                style={[
                  styles.filterButton,
                ...
                {selectedFilter === filter ? (
                  ...
                  >
                    <Text style={styles.activeFilterText}>
                      ...
                      {filter === 'strong_signal' && 'Signal Fort'}
                      {filter === 'weak_signal' && 'Signal Faible'}
                    </Text>
                  </LinearGradient>
                ) : (
            ...
            >
              ...
              >
                <View style={styles.braceletHeader}>
                  ...
                  {/* Indicateurs latéraux avec barres de signal */}
                  <View style={styles.indicatorsContainer}>
                    {/* Indicateur de proximité avec barres */}
                    <ProximityIndicator 
                ...
                <View style={styles.detailsContainer}>
                  ...
                  <View style={styles.detailsRow}>
                    <Text style={styles.detailsLabel}>Barres Signal:</Text>
                    <View style={styles.detailsBars}>
                      <SignalBars bars={bracelet.signal_bars} rssi={bracelet.rssi} distance={bracelet.distance_estimate} />
                    </View>
                  </View>
                  <View style={styles.detailsRow}>
      ...
      <View style={styles.container}>
        ...
        <View style={styles.loadingContainer}>
          ...
          <View style={styles.loadingInfo}>
            ...
            <Text style={styles.loadingInfoItem}>✅ Configuration barres de signal</Text>
          </View>
        </View>
      </View>
    );
...
const styles = StyleSheet.create({
  ...
  // Signal Bars Indicator (principal)
  signalBarsContainer: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: 8,
